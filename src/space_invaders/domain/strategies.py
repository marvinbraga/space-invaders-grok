...
class WaveDifficulty:
    ...
    def __init__(self, wave: int) -> None:
        ...
        self._wave = wave
        # Each wave is ~12% faster base step; fire more often
        self._wave_factor = 1.0 / (1.0 + 0.12 * (wave - 1))
        self._fire_factor = 1.0 / (1.0 + 0.15 * (wave - 1))
    ...
    def step_interval(self, alive_count: int, total: int) -> float:
        ...
        # When few remain, accelerate sharply
        if alive_count <= 1:
            return MIN_STEP_INTERVAL
        if alive_count <= 3:
            return max(MIN_STEP_INTERVAL, base * 0.12)
        if alive_count <= 8:
            return max(MIN_STEP_INTERVAL, base * 0.25)
        return max(MIN_STEP_INTERVAL, base * (0.35 + 0.65 * ratio))
    ...
    def ufo_speed(self) -> float:
        return UFO_SPEED * (1.0 + 0.05 * (self._wave - 1))

    def max_alien_bullets(self) -> int:
        return min(3, 1 + (self._wave - 1) // 2)
