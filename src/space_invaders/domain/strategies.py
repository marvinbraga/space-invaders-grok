...
class WaveDifficulty:
    ...
    def __init__(self, wave: int) -> None:
        ...
        self._wave = wave
        # Steeper wave ramp: ~22% faster march and denser fire per wave
        self._wave_factor = 1.0 / (1.0 + 0.22 * (wave - 1))
        self._fire_factor = 1.0 / (1.0 + 0.30 * (wave - 1))
    ...
    def step_interval(self, alive_count: int, total: int) -> float:
        ...
        if alive_count <= 1:
            return MIN_STEP_INTERVAL
        if alive_count <= 3:
            return max(MIN_STEP_INTERVAL, base * 0.06)
        if alive_count <= 8:
            return max(MIN_STEP_INTERVAL, base * 0.14)
        if alive_count <= 15:
            return max(MIN_STEP_INTERVAL, base * 0.24)
        if alive_count <= 25:
            return max(MIN_STEP_INTERVAL, base * 0.40)
    ...
    def ufo_speed(self) -> float:
        return UFO_SPEED * (1.0 + 0.10 * (self._wave - 1))

    def max_alien_bullets(self) -> int:
        # Wave 1 allows 3 concurrent shots; caps at 6
        return min(6, 3 + (self._wave - 1) // 1)
